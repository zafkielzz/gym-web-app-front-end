...
function baseValidate(fullInfo) {
  const errors = {};
...
export function loginValidate(fullInfo) {
  return baseValidate(fullInfo);
}

export function SignUpValidate(fullInfo) {
  const errors = baseValidate(fullInfo);

  if (fullInfo.username.trim().length < 4) {
    errors.username = "Tên đăng nhập phải có ít nhất 4 kí tự";
  }

  if (fullInfo.password.trim().length < 6) {
    errors.password = "Mật khẩu phải có ít nhất 6 kí tự";
  }
  if (!fullInfo.confirmedPassword.trim()) {
    errors.confirmedPassword = "Vui lòng xác nhận mật khẩu ";
  } else if (!(fullInfo.password === fullInfo.confirmedPassword)) {
    errors.confirmedPassword = "Mật khẩu không khớp";
  }
